  ...
  return (
    ...
    >
      <ScrollView
        ...
        // The pinned ActionBar below owns the bottom inset.
        ignoreBottomInset
      >
        {socialClients.isLoading ? (
          ...
          <Card style={styles.emptyBox}>
            <Text variant="h3">No clients with social accounts</Text>
      ...
      {/* Pinned rather than sitting at the end of the scroll: the primary
          action shouldn't require scrolling past every optional field, and
          ActionBar keeps it clear of the system navigation area. */}
      <ActionBar
        caption={
          selectedAccounts.length > 0 ? (
            ...
            <Text variant="caption" color="subtle" numberOfLines={1}>
              Choose a client and at least one account
            </Text>
          )
        }
      >
        <Button
          title={primaryLabel}
          icon={mode === 'now' ? 'send' : mode === 'draft' ? 'bookmark-outline' : 'time-outline'}
          block
      ...
      <Modal visible={!!publishProgress && publishing} transparent animationType="fade" statusBarTranslucent>
        <View style={[styles.modalBackdrop, { backgroundColor: t.scrim }]}>
          <Card elevation="xl" style={styles.modalCard}>
            <View style={styles.modalHead}>
              <View style={[styles.modalIcon, { backgroundColor: t.accent }]}>
                <Ionicons name="cloud-upload-outline" size={20} color={t.accentForeground} />
              </View>
              <View style={styles.modalTitle}>
                <Text variant="h3">Publishing</Text>
            ...
            <View style={styles.modalMeta}>
              <Text variant="caption" color="muted">
                {publishProgress?.done ?? 0} of {publishProgress?.total ?? 0} published
              </Text>
              {(publishProgress?.failed ?? 0) > 0 ? (
...
function AccountLogoButton({
  ...
  return (
    <PressableScale
      ...
      style={[styles.logoBtn, selected ? styles.logoOn : styles.logoOff]}
    >
      <SocialAccountAvatar
...
const styles = StyleSheet.create({
  ...
  form: { padding: spacing.gutter, gap: spacing.lg, paddingBottom: spacing.xl },
  section: { gap: spacing.sm },
  label: { ...type.label },
  primaryAction: { flex: 1 },
  mediaThumbs: { marginTop: spacing.sm },
  logoRow: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: spacing.md,
  ...
  // Selection is carried by opacity alone; scaling the avatar as well made
  // the row visibly reflow every time one was tapped.
  logoOn: { opacity: 1 },
  logoOff: { opacity: 0.45 },
  checkBadge: {
    position: 'absolute',
    top: -2,
    right: -2,
