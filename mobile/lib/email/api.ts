...
export const emailApi = {
  ...
  updateDraft: (
    id: string,
    data: Partial<Draft> & { to?: string[]; cc?: string[]; bcc?: string[] }
  ...
  trackingSummary: (mailboxId?: string) =>
    apiFetch<TrackingSummary>(
      `/email/tracking/summary${mailboxId ? `?mailboxId=${mailboxId}` : ''}`
    ),

  // ─── Templates ─────────────────────────────────────────────────
  listTemplates: (category?: string) =>
    apiFetch<{ templates: EmailTemplate[] }>(
      `/email/templates${category ? `?category=${category}` : ''}`
    ),
  createTemplate: (data: Partial<EmailTemplate>) =>
    apiFetch<{ template: EmailTemplate }>('/email/templates', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  updateTemplate: (id: string, data: Partial<EmailTemplate>) =>
    apiFetch<{ template: EmailTemplate }>(`/email/templates/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    }),
  ...
  createLabel: (data: { name: string; color: string }) =>
    apiFetch<{ label: EmailLabel }>('/email/labels', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  updateLabel: (id: string, data: { name?: string; color?: string }) =>
    apiFetch<{ label: EmailLabel }>(`/email/labels/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    }),
  ...
  analyticsOverview: (mailboxId?: string) =>
    apiFetch<AnalyticsOverview>(
      `/email/analytics/overview${mailboxId ? `?mailboxId=${mailboxId}` : ''}`
    ),
  ...
  analyticsTopSenders: (mailboxId?: string) =>
    apiFetch<{ senders: TopSender[] }>(
      `/email/analytics/top-senders${mailboxId ? `?mailboxId=${mailboxId}` : ''}`
    ),
  analyticsByAgent: (mailboxId?: string) =>
    apiFetch<{ agents: AgentStat[] }>(
      `/email/analytics/by-agent${mailboxId ? `?mailboxId=${mailboxId}` : ''}`
    ),
  analyticsByDepartment: () =>
    apiFetch<{ departments: DepartmentStat[] }>('/email/analytics/by-department'),
  analyticsActivity: (mailboxId?: string) =>
    apiFetch<{ activity: ActivityItem[] }>(
      `/email/analytics/activity${mailboxId ? `?mailboxId=${mailboxId}` : ''}`
    ),
  ...
  getCustomer: (conversationId: string) =>
    apiFetch<{ customer: CustomerContext | null; suggested: boolean }>(
      `/email/conversations/${conversationId}/customer`
    ),
  linkClient: (conversationId: string, clientId: string | null) =>
    apiFetch<{ success: boolean; clientId: string | null }>(
      `/email/conversations/${conversationId}/link-client`,
  ...
  // ─── Mail configuration health ─────────────────────────────────
  mailSettings: () =>
    apiFetch<{ canSend?: boolean; canReceive?: boolean }>('/settings/email'),
};
