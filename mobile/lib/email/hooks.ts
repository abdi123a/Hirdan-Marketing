...
export function useMailboxPermissions(mailboxId: string | null) {
  return useQuery({
...
export function useConversationActions() {
  ...
  const markUnread = useMutation({
    mutationFn: (id: string) => emailApi.markUnread(id),
    onSuccess: invalidateLists,
  });
  ...
  const patch = useMutation({
    ...
    onError: (e: Error) => toast.error(e.message || 'Failed to update conversation'),
  });
  ...
  return { markRead, markUnread, patch, action, bulk };
}
...
export function useForward() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ emailId, payload }: { emailId: string; payload: Parameters<typeof emailApi.forward>[1] }) =>
      emailApi.forward(emailId, payload),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['email', 'conversations'] });
      toast.success('Message forwarded');
    },
    onError: (err: Error) => toast.error(err.message || 'Failed to forward message'),
  });
}
...
/** Mail configuration health, used for the "email not configured" banner. */
export function useMailConfigIssue() {
  const { data } = useQuery({
    queryKey: ['email', 'mail-settings'],
    queryFn: () => emailApi.mailSettings(),
    staleTime: 5 * 60_000,
    // Non-admins cannot read /settings/email; a failure just hides the banner.
    retry: false,
  });

  if (!data) return null;
  if (!data.canSend && !data.canReceive) {
    return 'Email is not configured — sending and receiving are both blocked until Resend credentials are saved.';
  }
  if (!data.canReceive) {
    return 'Inbound email is blocked — add the Resend webhook signing secret in Settings → Email so Email Center can receive mail.';
  }
  if (!data.canSend) {
    return 'Outbound email is not configured — add a Resend API key and From address in Settings → Email.';
  }
  return null;
}
