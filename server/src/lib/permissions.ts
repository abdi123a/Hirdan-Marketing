/**
 * Enterprise-style module permissions.
 *
 * Levels (ascending):
 *   NONE   — no access (hidden from sidebar + blocked on API)
 *   READ   — view only
 *   WRITE  — create / update
 *   MANAGE — full control including delete / admin actions
 *
 * ADMIN role always bypasses and receives MANAGE on every module.
 * Other roles start from role defaults; per-user overrides live in User.permissions JSON.
 */
...
/** Sensible enterprise defaults per role. Admins always get ALL_MANAGE regardless. */
...
/** Merge role defaults with optional per-user overrides. ADMIN always full. */
export function resolvePermissions(
  ...
  // Ensure every module has a value
...
/** Sanitize incoming permission map from API body. */
export function sanitizePermissionMap(input: unknown): PermissionMap {
  if (!input || typeof input !== 'object' || Array.isArray(input)) return {};
  const result: PermissionMap = {};
  for (const mod of PERMISSION_MODULES) {
    const level = (input as Record<string, unknown>)[mod.key];
    if (isAccessLevel(level)) {
      result[mod.key] = level;
    }
  }
  return result;
}

/** Map a dashboard pathname to its permission module. */
export function moduleForPath(pathname: string): ModuleKey | null {
  const normalized = pathname.replace(/\/+$/, '') || '/dashboard';

  // Longest prefix match so /dashboard/social-media/presentation beats /dashboard/social-media
