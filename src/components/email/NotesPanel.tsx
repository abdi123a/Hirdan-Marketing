...
import { StickyNote, Send, Trash2, Loader2, AtSign, ChevronDown } from 'lucide-react';
...
export function NotesPanel({ conversationId, notes }: { conversationId: string; notes: ConversationNote[] }) {
  ...
  return (
    ...
    >
      ...
      >
        <StickyNote className={cn('h-4 w-4', open ? 'text-amber-600 dark:text-amber-500' : 'text-muted-foreground')} />
        <span className={cn('text-sm font-semibold', open ? 'text-amber-800 dark:text-amber-300' : 'text-foreground')}>
          Internal notes
        </span>
        {notes.length > 0 && (
          <span className="rounded-full bg-amber-200/80 px-1.5 py-0.5 text-[10px] font-semibold text-amber-900 dark:bg-amber-900/60 dark:text-amber-200">
            {notes.length}
          </span>
        )}
      ...
      {open && (
        <>
          ...
          <div className="relative px-3 pb-3 pt-1">
            <p className="mb-1.5 text-[11px] text-amber-700/70 dark:text-amber-500/70">
