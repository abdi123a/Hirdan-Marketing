...
import { ArrowLeft, Save, Folder, Users, DollarSign, Calendar, Tag, FileText } from "lucide-react";
...
export default function AddProjectPage() {
  ...
  const { clients, addProject } = useAgencyStore();
  ...
  const set = (field: keyof Project, value: string | number | string[]) =>
  ...
  const handleSubmit = async () => {
    if (!validate()) return;
    try {
      await addProject(form as Omit<Project, "id">);
      toast({ title: "Project created!", description: `"${form.name}" has been added to your projects.` });
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-foreground tracking-tight">New Project</h1>
          <p className="text-muted-foreground mt-0.5">Define the scope and details for the new project</p>
        </div>
      </div>
