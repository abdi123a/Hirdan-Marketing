...
export default function DashboardOverview() {
  ...
  const revenueTrend = useMemo(() => {
    ...
    return months.map((m, i) => {
      ...
      const paid = monthInvoices.filter(inv => inv.status === 'Paid').reduce((sum, inv) => sum + parseCurrency(inv.amount), 0);
      const recurring = subscriptions.filter(s => s.status === 'Active' && new Date(s.started).getMonth() <= i).reduce((sum, s) => {
        const amt = parseCurrency(s.amount);
        if (s.billingCycle === 'Annual') return sum + amt / 12;
        return sum + amt;
      }, 0);
      return { month: m, paid, recurring };
    });
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Recurring</span>
                </div>
              </div>
            </div>
            <div className="h-[350px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={revenueTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 11, fontWeight: 600, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} tickFormatter={(v) => `$${v/1000}k`} />
                  <Tooltip
